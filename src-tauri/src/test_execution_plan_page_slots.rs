use crate::new_architecture::actors::types::{ExecutionPlan, PageRange, PlanInputSnapshot, ExecutionPlanKpi, PageSlot};
use chrono::Utc;
...
mod tests {
    ...
    fn init_tracing() {
        ...
        ONCE.call_once(|| {
            let _ = set_global_default(FmtSubscriber::builder().with_max_level(Level::INFO).finish());
        });
    }

    fn build_plan(total_site_pages: u32, products_on_last_page: u32, ranges: Vec<PageRange>) -> ExecutionPlan {
        // Simplified snapshot
        ...
        for range in &ranges {
            let pages_iter: Box<dyn Iterator<Item=u32>> = if range.reverse_order { Box::new(range.end_page..=range.start_page) } else { Box::new(range.start_page..=range.end_page) };
            for physical_page in pages_iter {
                ...
                let capacity = if physical_page == total_site_pages { products_on_last_page.max(1) } else { crate::domain::constants::site::PRODUCTS_PER_PAGE as u32 };
                for offset in 0..capacity { let reverse_index = (capacity - 1 - offset) as i16; page_slots.push(PageSlot { physical_page, page_id, index_in_page: reverse_index }); }
            }
        }
        ExecutionPlan {
            plan_id: "test".into(),
            ...
            kpi_meta: Some(ExecutionPlanKpi { total_ranges: 1, total_pages: total_site_pages, batches: 1, strategy: "TestStrategy".into(), created_at: Utc::now() }),
    ...
    let range = PageRange { start_page: 5, end_page: 1, estimated_products: 5 * crate::domain::constants::site::PRODUCTS_PER_PAGE as u32, reverse_order: true };
        ...
        // Check a known page
        let p3: Vec<&PageSlot> = plan.page_slots.iter().filter(|s| s.physical_page == 3).collect();
        assert!(!p3.is_empty());
    ...
    let range = PageRange { start_page: 10, end_page: 8, estimated_products: 3 * crate::domain::constants::site::PRODUCTS_PER_PAGE as u32, reverse_order: true };
        let plan = build_plan(10, 7, vec![range]);
        // Slots for physical_page 10 should have capacity 7
        let p10: Vec<&PageSlot> = plan.page_slots.iter().filter(|s| s.physical_page == 10).collect();
        assert_eq!(p10.len(), 7, "Last page should have products_on_last_page count");
    ...
    let range = PageRange { start_page: 10, end_page: 6, estimated_products: 5 * crate::domain::constants::site::PRODUCTS_PER_PAGE as u32, reverse_order: true };
        ...
        for slot in &plan.page_slots { if !pairs.iter().any(|(p,_)| *p == slot.physical_page) { pairs.push((slot.physical_page, slot.page_id)); } }
        ...
        for (phys, pid) in pairs { assert_eq!(pid, (15 - phys) as i64); }
    }
    ...
    let range = PageRange { start_page: 3, end_page: 5, estimated_products: 3 * crate::domain::constants::site::PRODUCTS_PER_PAGE as u32, reverse_order: false };
        ...
        assert!(plan.page_slots.len() < original_len, "Tampering failed; test invalid");
        // Derive unique physical pages still present – should be subset but non-empty
        let unique_pages: std::collections::HashSet<u32> = plan.page_slots.iter().map(|s| s.physical_page).collect();
