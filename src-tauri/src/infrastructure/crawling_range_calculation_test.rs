...
mod tests {
    ...
    async fn test_crawling_range_calculation_prompts6_example() {
        ...
        // We need some products with max pageId=10 and max indexInPage=6
        let now = chrono::Utc::now().to_rfc3339();
        
        // Insert products from pageId 0 to 10
        for page_id in 0..=10 {
            ...
            for index_in_page in 0..=max_index {
                ...
                sqlx::query(r#"
                    INSERT INTO products (url, manufacturer, model, certificateId, pageId, indexInPage, createdAt, updatedAt)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                "#)
                .bind(&url)
