...
impl RetryManager {
    ...
    pub async fn add_failed_item(
        ...
        info!(
            "🔄 Adding failed item to retry queue: {} (stage: {:?})",
            item_id, stage
        );
        ...
        let classification = self
            .failure_classifier
            .classify_error(&error, stage.clone())
            .await;
        ...
        if !self
            .failure_classifier
            .should_retry(&classification, current_attempts)
            .await
        {
            warn!(
                "❌ Item {} cannot be retried: {:?}",
        ...
        let backoff = self
            .failure_classifier
            .calculate_backoff(current_attempts + 1)
            .await;
        ...
        // 기록 업데이트
        self.record_retry_attempt(&item_id, classification.clone(), backoff, false)
            .await;
    ...
    async fn record_retry_attempt(
        ...
        attempts.push(RetryAttempt {
            ...
            attempt_number: u32::try_from(attempts.len())
                .unwrap_or(u32::MAX)
                .saturating_add(1),
    ...
    async fn get_retry_count(&self, item_id: &str) -> u32 {
        ...
        history.get(item_id).map_or(0, |attempts| {
