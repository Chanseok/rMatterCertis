...
pub type RetryEventCallback =
    Arc<dyn Fn(String, String, String, u32, u32, String) -> Result<()> + Send + Sync>;
...
use crate::domain::services::crawling_services::{
    DatabaseAnalysis, DatabaseAnalyzer, ProductDetailCollector, ProductListCollector, SiteStatus,
    StatusChecker,
};
use crate::events::{AtomicTaskEvent, TaskStatus};
use crate::infrastructure::config::AppConfig;
use crate::infrastructure::crawling_service_impls::{
    CollectorConfig, CrawlingRangeCalculator, ProductDetailCollectorImpl, ProductListCollectorImpl,
    StatusCheckerImpl, product_detail_to_product,
};
use crate::infrastructure::system_broadcaster::SystemStateBroadcaster;
use crate::infrastructure::{
    HttpClient, IntegratedProductRepository, MatterDataExtractor, RetryManager,
};
...
impl BatchCrawlingConfig {
    ...
    pub fn update_range_from_calculation(&mut self, optimal_range: Option<(u32, u32)>) {
        if let Some((start_page, end_page)) = optimal_range {
            info!(
                "🔄 Updating crawling range from {}..{} to {}..{}",
            ...
            info!(
                "🔄 No optimal range available, keeping current range {}..{}",
...
impl DetailedCrawlingEvent {
    ...
    pub fn to_task_lifecycle_event(
        &self,
        session_id: &str,
    ) -> Option<(TaskExecutionContext, TaskLifecycleEvent)> {
        let now = Utc::now();
        ...
        match self {
            ...
            DetailedCrawlingEvent::PageRetryAttempt {
                page,
                batch_id,
                url,
                attempt,
            ...
            DetailedCrawlingEvent::ProductRetryAttempt {
                url,
                batch_id,
                attempt,
                max_attempts,
                ...
                let context = TaskExecutionContext {
                    ...
                    task_id: format!(
                        "product_retry_{}",
                        url.chars().rev().take(8).collect::<String>()
                    ),
                    task_url: url.clone(),
                    start_time: now,
                    worker_id: Some(format!("retry_worker_{}", attempt)),
...
impl ServiceBasedBatchCrawlingEngine {
    pub fn new(
        ...
        let product_list_collector: Arc<dyn ProductListCollector> =
        ...
        let product_detail_collector: Arc<dyn ProductDetailCollector> =
            Arc::new(ProductDetailCollectorImpl::new(
    ...
    pub async fn execute(&mut self) -> Result<()> {
        ...
        info!(
            "Starting service-based 4-stage batch crawling for session: {}",
            self.session_id
        );
        ...
        if let Some(cancellation_token) = &self.config.cancellation_token {
            if cancellation_token.is_cancelled() {
                ...
                return Err(anyhow!(
                    "Crawling session cancelled after site status check"
                ));
            }
        }
        ...
        let (actual_start_page, actual_end_page) = if self.config.disable_intelligent_range {
            ...
            info!(
                "🎭 Actor mode: Using exact user-specified range {} to {} (intelligent range disabled)",
                self.config.start_page, self.config.end_page
            );
            (self.config.start_page, self.config.end_page)
        } else {
            ...
            info!(
                "📊 Site analysis: total_pages={}, products_on_last_page={}",
                site_status.total_pages, site_status.products_on_last_page
            );

            let optimal_range = self
                .range_calculator
            ...
            if let Some((optimal_start, optimal_end)) = optimal_range {
                if optimal_start != self.config.start_page || optimal_end != self.config.end_page {
                    info!(
                        "💡 Applying intelligent range recommendation: pages {} to {} (original: {} to {})",
                    ...
                    self.emit_detailed_event(DetailedCrawlingEvent::StageStarted {
                        ...
                        message: format!(
                            "Applied optimal range: {} to {} (was: {} to {})",
                            optimal_start,
                            optimal_end,
                    ...
                    info!(
                        "✅ Current range already optimal: {} to {}",
                        self.config.start_page, self.config.end_page
                    );
                    (self.config.start_page, self.config.end_page)
                }
                ...
                info!(
                    "✅ All products appear to be crawled - using current range for verification: {} to {}",
                    self.config.start_page, self.config.end_page
                );
                (self.config.start_page, self.config.end_page)
            }
        ...
        info!(
            "🎯 Final crawling range determined: {} to {}",
            actual_start_page, actual_end_page
        );
        ...
        if let Some(cancellation_token) = &self.config.cancellation_token {
            if cancellation_token.is_cancelled() {
                ...
                return Err(anyhow!(
                    "Crawling session cancelled after database analysis"
                ));
            }
        }
        ...
        info!(
            "📋 배치 계획 수립: 총 {}페이지를 {}개 배치로 분할",
            total_pages, estimated_batches
        );
        ...
        for batch_num in 1..=estimated_batches {
            ...
            self.emit_detailed_event(DetailedCrawlingEvent::BatchCreated {
                ...
                description: format!(
                    "배치 {} (페이지 {}-{})",
                    batch_num, start_page_for_batch, end_page_for_batch
                ),
            })
            .await?;
        }
        ...
        let product_urls = self
            .stage2_collect_product_list_optimized(
                actual_start_page,
                actual_end_page,
                site_status.total_pages,
        ...
        // 🔥 Stage 2 결과 검증 및 로깅
        info!(
            "📊 Stage 2 completed: {} product URLs collected",
            product_urls.len()
        );
        if product_urls.is_empty() {
            warn!(
                "⚠️  No product URLs collected from Stage 2! This will prevent Stage 3 from running."
            );
            ...
            info!(
                "✅ Stage 2 successful: {} URLs ready for Stage 3",
                product_urls.len()
            );
            // 샘플 URL 로깅 (디버깅용)
            if !product_urls.is_empty() {
        ...
        if let Some(cancellation_token) = &self.config.cancellation_token {
            if cancellation_token.is_cancelled() {
                ...
                return Err(anyhow!(
                    "Crawling session cancelled after product list collection"
                ));
            }
        }
        ...
        info!(
            "🚀 Proceeding to Stage 3 with {} product URLs",
            product_urls.len()
        );
        ...
        if let Some(cancellation_token) = &self.config.cancellation_token {
            if cancellation_token.is_cancelled() {
                ...
                return Err(anyhow!(
                    "Crawling session cancelled after product details collection"
                ));
            }
        }

        // Stage 4: 데이터베이스 저장
        let (processed_count, _new_items, _updated_items, errors) =
        ...
        if let Some(broadcaster) = &mut self.broadcaster {
            ...
            if let Err(e) =
                broadcaster.emit_batch_completed(pages_processed, total_products, success_rate)
            {
        ...
        info!(
            "Service-based batch crawling completed in {:?}: {} products collected, {:.2}% success rate",
            duration,
            total_products,
            success_rate * 100.0
        );
    ...
    async fn stage0_check_site_status(&self) -> Result<SiteStatus> {
        ...
        if !site_status.is_accessible || site_status.total_pages == 0 {
            let error_msg = format!(
                "Site is not accessible or has no pages (pages: {})",
                site_status.total_pages
            );
        ...
        info!(
            "Stage 0 completed: Site is healthy (score: {})",
    ...
    async fn stage1_analyze_database(&self) -> Result<DatabaseAnalysis> {
        ...
        info!(
            "Stage 1 completed: {} total products, quality score: {}",
    ...
    async fn stage2_collect_product_list(
        &self,
        total_pages: u32,
        ...
        let product_urls = if let Some(cancellation_token) = &self.config.cancellation_token {
            ...
            self.product_list_collector
                .collect_page_range_with_cancellation(
                    self.config.start_page,
                    effective_end,
                    total_pages,
        ...
        if let Some(cancellation_token) = &self.config.cancellation_token {
            if cancellation_token.is_cancelled() {
                ...
                return Err(anyhow!(
                    "Product list collection cancelled after completion"
                ));
            }
        }
    ...
    async fn stage2_collect_product_list_optimized(
        &mut self,
        start_page: u32,
        end_page: u32,
        _total_pages: u32,
        ...
        info!(
            "🔗 Stage 2: ProductList 수집 시작 - 페이지별 병렬 실행 ({}~{})",
            start_page, end_page
        );
        ...
        info!(
            "📦 ProductList 배치 생성: {} ({}페이지)",
            batch_id, total_pages
        );
        if let Some(broadcaster) = &self.broadcaster {
        ...
        let (event_tx, mut event_rx) =
            tokio::sync::mpsc::channel::<(String, serde_json::Value)>(100);
                ...
                if let Some(ref mut b) = broadcaster {
                    match event_type.as_str() {
                        "page-collection-started" => {
                            if let Ok(detailed_event) =
                                serde_json::from_value::<DetailedCrawlingEvent>(payload)
                            {
                                // 새로운 page-collection-started 이벤트 처리
                                debug!(
                                    "Page collection started event received: {:?}",
                                    detailed_event
                                );
                                // 기존 브로드캐스터로 변환하여 전송
                                match &detailed_event {
                                    DetailedCrawlingEvent::PageCollectionStarted {
                                        page,
                                        url,
                                        ..
                                    } => {
                        ...
                        "page-collection-completed" => {
                                ...
                                serde_json::from_value::<DetailedCrawlingEvent>(payload)
                            {
                                ...
                                debug!(
                                    "Page collection completed event received: {:?}",
                                    detailed_event
                                );
                                // 기존 브로드캐스터로 변환하여 전송
                                match &detailed_event {
                                    DetailedCrawlingEvent::PageCollectionCompleted {
                                        page,
                                        url,
                                        products_found,
                                        ..
                                    } => {
                                        if let Err(e) = b.emit_page_crawled(
                                            *page,
                                            url.clone(),
                                            *products_found,
                                            true,
                                            ...
                                            warn!(
                                                "Failed to emit page-collection-completed as page-crawled event: {}",
                                                e
                                            );
                                        }
                                    }
                                    ...
                                    DetailedCrawlingEvent::PageCompleted {
                                        ...
                                        if let Err(e) = b.emit_page_crawled(
                                            ...
                                            *products_found,
                                            ...
                                            warn!(
                                                "Failed to emit page-completed as page-crawled event: {}",
                                                e
                                            );
                                        }
                                    }
                        ...
                        "page-crawled" => {
                            if let Ok(data) =
                                serde_json::from_value::<(u32, String, u32, bool)>(payload)
                            {
                        ...
                        "retry-attempt" => {
                            ...
                            {
                                if let Err(e) = b.emit_retry_attempt(
                        ...
                        "retry-success" => {
                            if let Ok(data) =
                                serde_json::from_value::<(String, String, String, u32)>(payload)
                            {
                        ...
                        "retry-failed" => {
                            if let Ok(data) =
                                serde_json::from_value::<(String, String, String, u32, String)>(
                                    payload,
                                )
                            {
                                if let Err(e) =
                                    b.emit_retry_failed(data.0, data.1, data.2, data.3, data.4)
                                {
        ...
        let _page_callback =
            move |page_id: u32, url: String, product_count: u32, success: bool| -> Result<()> {
                ...
                if let Err(e) =
                    event_tx_clone.try_send(("page-collection-started".to_string(), start_payload))
                {
                ...
                if success {
                    ...
                    if let Err(e) =
                        event_tx_clone.try_send(("page-collection-completed".to_string(), payload))
                    {
                ...
                if let Err(e) =
                    event_tx_clone.try_send(("page-crawled".to_string(), legacy_payload))
                {
        ...
        let _retry_callback = move |item_id: String,
                                    item_type: String,
                                    url: String,
                                    attempt: u32,
                                    max_attempts: u32,
            ...
            if item_type == "page" {
                ...
                if let Err(e) =
                    event_tx_clone2.try_send(("page-retry-attempt".to_string(), retry_payload))
                {
            ...
            // 기존 재시도 이벤트
            let payload =
                serde_json::to_value((item_id, item_type, url, attempt, max_attempts, reason))?;
        ...
        let product_urls = if let Some(cancellation_token) = &self.config.cancellation_token {
            ...
            collector_impl
                .collect_page_range_with_async_events(
                    start_page,
                    end_page,
            ...
            collector_impl
                ...
                .await
                .map_err(|e| anyhow!("Product list collection failed: {}", e))?
        };
        ...
        info!(
            "✅ Stage 2 completed: {} product URLs collected from optimized range with TRUE concurrent execution",
            product_urls.len()
        );
    ...
    async fn stage3_collect_product_details(
        &mut self,
        product_urls: &[ProductUrl],
    ) -> Result<Vec<(Product, ProductDetail)>> {
        info!(
            "Stage 3: Collecting product details using ProductDetailCollector service with retry mechanism"
        );
        ...
        let result = if let Some(cancellation_token) = &self.config.cancellation_token {
            ...
            if let Some(collector_impl) = self
                .product_detail_collector
                .as_any()
                .downcast_ref::<ProductDetailCollectorImpl>()
            {
                collector_impl
                    .collect_details_with_async_events(
                        product_urls,
                        Some(cancellation_token.clone()),
                        self.session_id.clone(),
                ...
                self.product_detail_collector
                    .collect_details_with_cancellation(product_urls, cancellation_token.clone())
                    .await
            }
        } else {
                ...
                .downcast_ref::<ProductDetailCollectorImpl>()
            {
                ...
                // Fallback to original method
                self.product_detail_collector
                    .collect_details_with_cancellation(product_urls, default_token)
                    .await
            }
        };
        ...
        match result {
            Ok(product_details) => {
                ...
                successful_products = product_details
                    .into_iter()
                    .enumerate()
                    .map(|(index, detail)| {
                        ...
                        if let Some(product_url) = product_urls.get(index) {
                            ...
                            let _completion_event =
                                DetailedCrawlingEvent::ProductDetailCollectionCompleted {
                                    url: product_url.to_string(),
                                    product_index: (index + 1) as u32,
                                    success: true,
                                    duration_ms,
                                    data_extracted: detail.model.is_some()
                                        && detail.manufacturer.is_some(),
                                };
                            ...
                            if let Some(broadcaster) = &mut self.broadcaster {
                                if let Err(e) = broadcaster.emit_product_collected(
                                    ...
                                    product
                                        .model
                                        .clone()
                                        .unwrap_or_else(|| format!("product-{}", index)),
                ...
                info!(
                    "✅ Initial collection successful: {} products",
                ...
                for (index, url) in failed_urls.iter().enumerate() {
                    if let Some(broadcaster) = &mut self.broadcaster {
                        if let Err(emit_err) = broadcaster.emit_product_collected(
                            ...
                            warn!(
                                "Failed to emit product-collected failure event: {}",
                                emit_err
                            );
                        }
                    }
        ...
        info!(
            "Stage 3 completed: {} products collected (including retries)",
            successful_products.len()
        );
    ...
    /// 제품 상세정보 수집 재시도 처리
    async fn process_retries_for_product_details(
        &mut self,
    ) -> Result<Vec<(Product, ProductDetail)>> {
        ...
        for cycle in 1..=3 {
            ...
            for retry_item in ready_items {
                ...
                if retry_item.stage == CrawlingStage::ProductDetails {
                    ...
                    .await
                    .unwrap_or_else(|e| warn!("Failed to emit ProductRetryAttempt event: {}", e));
                        ...
                        .await
                    {
                        Ok(mut product_details) => {
                            if let Some(detail) = product_details.pop() {
                                ...
                                self.emit_detailed_event(
                                    DetailedCrawlingEvent::ProductRetrySuccess {
                                        url: url.clone(),
                                        batch_id: 1,
                                        final_attempt: cycle,
                                    },
                                ...
                                .await
                                .unwrap_or_else(|e| {
                                    warn!("Failed to emit ProductRetrySuccess event: {}", e)
                                });
                                ...
                                // 성공 기록
                                if let Err(e) =
                                    self.retry_manager.mark_retry_success(&item_id).await
                                {
                        ...
                        Err(e) => {
                                ...
                                .await
                            {
                                ...
                                self.emit_detailed_event(
                                    DetailedCrawlingEvent::ProductRetryFailed {
                                        url: url.clone(),
                                        batch_id: 1,
                                ...
                                .await
                                .unwrap_or_else(|e| {
                                    warn!("Failed to emit ProductRetryFailed event: {}", e)
                                });
        ...
        info!(
            "🔄 Retry processing completed: {} additional products collected",
            retry_products.len()
        );
        Ok(retry_products)
    }
    ...
    async fn stage4_save_to_database(
        ...
        info!(
            "Stage 4: Batch saving {} products to database",
            products.len()
        );
        ...
        info!(
            "📦 Processing {} products in {} batches of {} items each",
        ...
        for (batch_index, batch) in product_batches.into_iter().enumerate() {
            ...
            if let Some(cancellation_token) = &self.config.cancellation_token {
                if cancellation_token.is_cancelled() {
                    warn!(
                        "🛑 Database batch save cancelled after {} batches",
                        batch_index
                    );
                    break;
                }
            ...
            for (product, product_detail) in batch.iter() {
                ...
                let product_detail_save_result = self
                    .product_detail_repo
                ...
                match (product_save_result, product_detail_save_result) {
                    (
                        Ok((product_was_updated, product_was_created)),
                        Ok((detail_was_updated, detail_was_created)),
                    ) => {
            ...
            info!(
                "✅ 배치 {}/{} 완료: {}개 저장 (신규: {}, 업데이트: {}, 오류: {}) in {}ms",
                batch_id,
                total_batches,
                batch.len() - batch_errors,
                batch_new_items,
                batch_updated_items,
                batch_errors,
                batch_duration_ms
    ...
    async fn emit_detailed_event(&self, event: DetailedCrawlingEvent) -> Result<()> {
        ...
        if let Some(emitter) = self.event_emitter.as_ref() {
            ...
            if let Some(concurrency_event) = concurrency_event {
                if let Ok(json_value) = serde_json::to_value(&concurrency_event) {
                    emitter
                        .emit_detailed_crawling_event_json(json_value)
                        .await?;
                }
            }
        }
        ...
        if let Some(emitter) = self.event_emitter.as_ref() {
            ...
            let progress = match &event {
                DetailedCrawlingEvent::StageStarted { stage, message } => CrawlingProgress {
                    current: 0,
                ...
                DetailedCrawlingEvent::ProductProcessed { url, success } => CrawlingProgress {
                    current: 1,
                    total: 1,
                ...
                DetailedCrawlingEvent::BatchCompleted { batch, total } => CrawlingProgress {
                    current: *batch,
                    total: *total,
                ...
                DetailedCrawlingEvent::SessionStarted {
                    session_id,
                    config: _,
                } => CrawlingProgress {
                    current: 0,
                ...
                DetailedCrawlingEvent::SessionCompleted {
                    ...
                    message: format!(
                        "Session {} completed: {} products, {:.1}% success rate",
                        session_id,
                        total_products,
                        success_rate * 100.0
                    ),
                ...
                DetailedCrawlingEvent::BatchCreated {
                    ...
                    current_step: format!(
                        "배치 {}/{} 생성: {}",
                        batch_id, total_batches, description
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Batch {}/{} created: pages {} to {}",
                        batch_id, total_batches, start_page, end_page
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                    new_items: 0,
                ...
                DetailedCrawlingEvent::BatchStarted {
                    ...
                    current_step: format!(
                        "배치 {}/{} 시작 ({}개 페이지)",
                        batch_id, total_batches, pages_in_batch
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Batch {}/{} started: {} pages",
                ...
                DetailedCrawlingEvent::PageRetryAttempt {
                    ...
                    current_step: format!(
                        "페이지 {} 재시도 {}/{} (배치 {}) - {}",
                        page, attempt, max_attempts, batch_id, reason
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Page {} attempt {}/{} in batch {} - {}",
                        page, attempt, max_attempts, batch_id, reason
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::PageRetrySuccess {
                    page,
                    batch_id,
                    url: _,
                    final_attempt,
                    ...
                    current_stage: CrawlingStage::ProductList,
                    current_step: format!(
                        "페이지 {} 재시도 성공 ({}번째 시도, {}개 제품, 배치 {})",
                        page, final_attempt, products_found, batch_id
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Page attempt succeeded on attempt {} with {} products (batch {})",
                        final_attempt, products_found, batch_id
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                    new_items: *products_found,
                ...
                DetailedCrawlingEvent::PageRetryFailed {
                    page,
                    batch_id,
                    url: _,
                    total_attempts,
                    ...
                    current_stage: CrawlingStage::ProductList,
                    current_step: format!(
                        "페이지 {} 최종 실패 ({}번 재시도 후, 배치 {}) - {}",
                        page, total_attempts, batch_id, final_error
                    ),
                    status: CrawlingStatus::Error,
                    message: format!(
                        "Page {} finally failed after {} attempts (batch {}) - {}",
                        page, total_attempts, batch_id, final_error
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductStarted {
                    ...
                    current_step: format!(
                        "제품 {}/{} 시작 (배치 {})",
                        product_index, total_products, batch_id
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Product {}/{} started in batch {}",
                        product_index, total_products, batch_id
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductRetryAttempt {
                    ...
                    current_step: format!(
                        "제품 재시도 {}/{} (배치 {}) - {}",
                        attempt, max_attempts, batch_id, reason
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Product attempt {}/{} in batch {} - {}",
                        attempt, max_attempts, batch_id, reason
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductRetrySuccess {
                    ...
                    current_step: format!(
                        "제품 재시도 성공 ({}번째 시도, 배치 {})",
                        final_attempt, batch_id
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Product attempt succeeded on attempt {} (batch {})",
                        final_attempt, batch_id
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductRetryFailed {
                    ...
                    current_step: format!(
                        "제품 최종 실패 ({}번 재시도 후, 배치 {}) - {}",
                        total_attempts, batch_id, final_error
                    ),
                    status: CrawlingStatus::Error,
                    message: format!(
                        "Product finally failed after {} attempts (batch {}) - {}",
                        total_attempts, batch_id, final_error
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::StageCompleted {
                    ...
                    status: CrawlingStatus::Completed,
                    message: format!(
                        "Stage {} completed: {} items processed",
                        stage, items_processed
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::PageCollectionStarted {
                    ...
                    current_stage: CrawlingStage::ProductList,
                    current_step: format!(
                        "페이지 {} 수집 시작 (배치 {}, 예상 제품: {}개)",
                        page,
                        batch_id,
                        estimated_products.unwrap_or(0)
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Page {} collection started (batch {}, estimated: {})",
                        page,
                        batch_id,
                        estimated_products.unwrap_or(0)
                    ),
                ...
                DetailedCrawlingEvent::PageCollectionCompleted {
                    ...
                    current_stage: CrawlingStage::ProductList,
                    current_step: format!(
                        "페이지 {} 수집 완료: {}개 제품 발견 ({}ms, 배치 {})",
                        page, products_found, duration_ms, batch_id
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Page {} collection completed: {} products found in {}ms (batch {})",
                        page, products_found, duration_ms, batch_id
                    ),
                    remaining_time: None,
                ...
                // 🔥 새로운 제품 상세 수집 이벤트들
                DetailedCrawlingEvent::ProductDetailCollectionStarted {
                    url: _,
                    product_index,
                    total_products,
                    batch_id,
                    ...
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Product detail collection started: {}/{} (batch {})",
                        product_index, total_products, batch_id
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductDetailProcessingStarted {
                    url: _,
                    product_index,
                    parsing_stage,
                } => CrawlingProgress {
                    ...
                    current_step: format!(
                        "제품 {} 상세정보 처리 시작: {}",
                        product_index, parsing_stage
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Product {} detail processing started: {}",
                        product_index, parsing_stage
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::ProductDetailCollectionCompleted {
                    ...
                    message: format!(
                        "Product {} detail collection completed: {} (data extracted: {}) in {}ms",
                        product_index,
                ...
                DetailedCrawlingEvent::DatabaseBatchSaveStarted {
                    batch_id,
                    products_count,
                    batch_size,
                } => CrawlingProgress {
                    ...
                    current_stage: CrawlingStage::DatabaseSave,
                    current_step: format!(
                        "데이터베이스 배치 {} 저장 시작 ({}개 제품, 배치 크기: {})",
                        batch_id, products_count, batch_size
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Database batch {} save started: {} products (batch size: {})",
                        batch_id, products_count, batch_size
                    ),
                    remaining_time: None,
                    elapsed_time: 0,
                ...
                DetailedCrawlingEvent::DatabaseBatchSaveCompleted {
                    batch_id,
                    products_saved,
                    new_items,
                    updated_items,
                    ...
                    current_stage: CrawlingStage::DatabaseSave,
                    current_step: format!(
                        "데이터베이스 배치 {} 저장 완료: {}개 저장 (신규: {}, 업데이트: {}, 오류: {}) in {}ms",
                        batch_id, products_saved, new_items, updated_items, errors, duration_ms
                    ),
                    status: CrawlingStatus::Running,
                    message: format!(
                        "Database batch {} save completed: {} saved (new: {}, updated: {}, errors: {}) in {}ms",
                        batch_id, products_saved, new_items, updated_items, errors, duration_ms
                    ),
                    remaining_time: None,
                    elapsed_time: *duration_ms,
    ...
    pub fn update_cancellation_token(&mut self, cancellation_token: Option<CancellationToken>) {
        ...
        info!(
            "🔄 Updated cancellation token in ServiceBasedBatchCrawlingEngine: {}",
...
impl std::fmt::Debug for ServiceBasedBatchCrawlingEngine {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ServiceBasedBatchCrawlingEngine")
            ...
            .field(
                "product_detail_collector",
                &"Arc<dyn ProductDetailCollector>",
            )
