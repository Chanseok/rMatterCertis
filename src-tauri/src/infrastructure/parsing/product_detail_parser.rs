...
impl ProductDetailParser {
    ...
    pub fn with_config(selectors: &super::config::ProductDetailSelectors) -> Result<Self> {
        ...
        Ok(Self {
            ...
            specification_version_selectors: Self::compile_selectors(
                &selectors.specification_version,
            )?,
...
impl ContextualParser for ProductDetailParser {
    ...
    fn parse_with_context(
        ...
        let model = self
            .extract_basic_info(html, "title", &self.title_selectors)
            .or_else(|| self.extract_basic_info(html, "model", &self.model_selectors))
            .ok_or_else(|| {
                ParsingError::required_field_missing("model", Some("product detail page"))
            })?;

        let manufacturer =
            self.extract_basic_info(html, "manufacturer", &self.manufacturer_selectors);
...
impl ProductDetailParser {
    ...
    fn extract_basic_info(
        ...
        debug!(
            "Failed to extract {} using {} selectors",
            field_name,
            selectors.len()
        );
        None
    ...
    fn extract_matter_certification_data(
        ...
        if let Some(labeled_data) = self.extract_from_labeled_elements(html) {
            debug!(
                "Extracted {} fields from labeled elements",
                labeled_data.len()
            );
...
mod tests {
    ...
    fn test_numeric_field_parsing() {
        ...
        assert_eq!(
            parser.parse_numeric_field(Some(&"0x1234".to_string())),
            Some(4660)
        );
        assert_eq!(
            parser.parse_numeric_field(Some(&"1234".to_string())),
            Some(4660)
        ); // hex로 파싱됨
        assert_eq!(
            parser.parse_numeric_field(Some(&"ABCD".to_string())),
            Some(0xABCD)
        );
        assert_eq!(
