...
use super::{ContextualParser, ParseContext, ParsingError, ParsingResult};
...
impl ContextualParser for ProductListParser {
    ...
    fn parse_with_context(
        ...
        if products.is_empty() {
            return Err(ParsingError::no_products_found(
                context.page_id,
                tried_selectors,
            ));
        }
...
impl ProductListParser {
    ...
    fn extract_product_from_element(
        ...
        let model_name = self
            .extract_text_with_fallbacks(element, &self.title_selectors)
            .ok_or_else(|| {
                ParsingError::required_field_missing("model_name", Some("product listing"))
            })?;

        // Extract brand with fallbacks (allow empty)
        let brand = self
            .extract_text_with_fallbacks(element, &self.brand_selectors)
            .unwrap_or_else(|| "Unknown".to_string());

        // Extract category/certificate_id with fallbacks (allow empty)
        let category = self
            .extract_text_with_fallbacks(element, &self.category_selectors)
            .unwrap_or_else(|| "Unknown".to_string());
        ...
        Ok(Product {
            ...
            url,
            manufacturer: Some(brand.trim().to_string())
