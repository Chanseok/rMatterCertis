...
impl AppConfig {
    ...
    pub fn create_http_client(
        ...
        crate::infrastructure::simple_http_client::HttpClient::from_worker_config(
            &self.user.crawling.workers,
        )
    }
}
...
impl ConfigManager {
    ...
    pub fn get_config_dir() -> Result<PathBuf> {
        ...
        if app_support_config.exists() {
            debug!(
                "Using configuration from Application Support: {:?}",
                app_support_dir
            );
        ...
        if config_config.exists() {
            info!(
                "Using configuration from config directory: {:?}",
                config_dir
            );
        ...
        info!(
            "No existing config found, defaulting to Application Support: {:?}",
            app_support_dir
        );
        Ok(app_support_dir)
    }
    ...
    fn migrate_config_from_old_location(target_config_path: &PathBuf) -> Result<()> {
        ...
        if old_config_path.exists() {
            info!(
                ...
                old_config_path, target_config_path
            );
    ...
    async fn migrate_config_format(&self, content: &str) -> Result<AppConfig> {
        ...
        if let Some(user_obj) = json_value.get_mut("user").and_then(|v| v.as_object_mut()) {
            ...
            if let Some(crawling) = user_obj.get_mut("crawling").and_then(|v| v.as_object_mut()) {
                if !crawling.contains_key("validation_page_limit") {
                    crawling.insert(
                        "validation_page_limit".into(),
                        serde_json::Value::from(50u32),
                    );
        ...
        let migrated_config: AppConfig =
            serde_json::from_value(json_value).context("Failed to parse migrated configuration")?;
...
pub mod defaults {
    ...
    pub const USER_AGENT: &str =
        "matter-certis-v2/1.0 (Research Tool; +https://github.com/your-repo)";
