...
use crate::domain::services::{
    BatchProgressTracker, BatchRecoveryService, ConflictResolver, DatabaseAnalyzer,
    DeduplicationService, ErrorClassifier, ProductDetailCollector, ProductListCollector,
    StatusChecker, ValidationService,
};
...
use crate::infrastructure::data_processing_service_impls::{
    BatchProgressTrackerImpl, BatchRecoveryServiceImpl, ErrorClassifierImpl, RetryManagerImpl,
};
use crate::infrastructure::service_based_crawling_engine::{
    BatchCrawlingConfig, DetailedCrawlingEvent,
};
use crate::infrastructure::{
    CollectorConfig, ConflictResolverImpl, DeduplicationServiceImpl, HttpClient,
    IntegratedProductRepository, MatterDataExtractor, ProductListCollectorImpl, StatusCheckerImpl,
    ValidationServiceImpl, config::AppConfig,
};
...
impl AdvancedBatchCrawlingEngine {
    pub fn new(
        ...
        let database_analyzer: Arc<dyn DatabaseAnalyzer> =
            Arc::new(StatusCheckerImpl::with_product_repo(
                http_client.clone(),
                data_extractor.clone(),
                app_config.clone(),
        ...
        let deduplication_service: Arc<dyn DeduplicationService> =
            Arc::new(DeduplicationServiceImpl::new(0.85));
        let validation_service: Arc<dyn ValidationService> = Arc::new(ValidationServiceImpl::new());
        let conflict_resolver: Arc<dyn ConflictResolver> = Arc::new(ConflictResolverImpl::new(
            ResolutionStrategy::KeepMostComplete,
        ));

        // 고급 관리 서비스 인스턴스 생성
        let progress_tracker: Arc<dyn BatchProgressTracker> =
            Arc::new(BatchProgressTrackerImpl::new());
        let recovery_service: Arc<dyn BatchRecoveryService> =
            Arc::new(BatchRecoveryServiceImpl::new());
    ...
    pub async fn execute(&self) -> Result<()> {
        ...
        info!(
            "🚀 Starting advanced batch crawling with STRICT CONFIG LIMITS for session: {}",
            self.session_id
        );
        info!(
            "📊 Config limits: start_page={}, end_page={}, batch_size={}, concurrency={}",
        ...
        self.progress_tracker
            .update_progress(
                &batch_id,
                crate::domain::services::data_processing_services::BatchProgress {
        ...
        match self.execute_with_error_handling(&batch_id).await {
            Ok((products_count, calculated_success_rate)) => {
                ...
                self.progress_tracker
                    .complete_batch(&batch_id, batch_result)
                    .await?;
            }
            Err(e) => {
                ...
                let _severity = self
                    .error_classifier
                    .assess_severity(&e.to_string())
                    .await?;
                let is_recoverable = self
                    .error_classifier
                    .assess_recoverability(&e.to_string())
                    .await?;
                ...
                if is_recoverable {
                    ...
                    match self
                        .recovery_service
                        .recover_parsing_error(&e.to_string())
                        .await
                    {
                        Ok(recovery_action) => {
    ...
    async fn execute_with_error_handling(&self, batch_id: &str) -> Result<(u32, f64)> {
        ...
        let product_urls = self
            .stage2_collect_product_list(site_status.total_pages)
            .await?;
        ...
        let (processed_count, _new_items, _updated_items, errors) =
            self.stage5_save_to_database(processed_products).await?;
    ...
    async fn stage2_collect_product_list(&self, total_pages: u32) -> Result<Vec<ProductUrl>> {
        ...
        info!(
            "   - Configuration: start_page={}, end_page={}",
            start_page, end_page
        );
        info!("   - Site total_pages={}", total_pages);
        info!(
            "   - Pages to process: {} (from {} to {})",
            actual_pages_to_process, start_page, end_page
        );
        info!(
            "   - Collection order: {}",
            if start_page > end_page {
                "oldest first (descending)"
            } else {
        ...
        info!(
            "✅ Stage 2 completed: {} product URLs collected from pages {}-{} (range enforced)",
            product_urls.len(),
            start_page,
            end_page
        );
    ...
    async fn stage3_collect_product_details(
        ...
        let products: Vec<Product> = product_details
            .into_iter()
            .map(|detail| {
                crate::infrastructure::crawling_service_impls::product_detail_to_product(detail)
            })
            .collect();
    ...
    async fn stage4_process_data_pipeline(
        &self,
        ...
        let deduplication_analysis = self
            .deduplication_service
            .analyze_duplicates(&raw_products)
            .await?;
        info!(
            "Duplicate analysis: {:.2}% duplicates found",
            deduplication_analysis.duplicate_rate * 100.0
        );

        let deduplicated_products = self
            .deduplication_service
            .remove_duplicates(raw_products)
            .await?;
        info!(
            "Deduplication completed: {} products remaining",
            deduplicated_products.len()
        );
        ...
        let validation_result = self
            .validation_service
            .validate_all(deduplicated_products)
            .await?;
        info!(
            "Validation completed: {} valid, {} invalid products",
            validation_result.valid_products.len(),
            validation_result.invalid_products.len()
        );
            ...
            .is_empty()
        {
            info!(
                ...
                validation_result.validation_summary.common_errors
            );
        }

        // 4.3: 충돌 해결
        info!("Step 4.3: Resolving conflicts");
        let resolved_products = self
            .conflict_resolver
            .resolve_conflicts(validation_result.valid_products)
            .await?;
        info!(
            "Conflict resolution completed: {} final products",
            resolved_products.len()
        );
        ...
        info!(
            "Stage 4 completed: Data processing pipeline finished with {} high-quality products",
            resolved_products.len()
        );
    ...
    /// Stage 5: 데이터베이스 저장
    async fn stage5_save_to_database(
        &self,
        products: Vec<Product>,
    ) -> Result<(usize, usize, usize, usize)> {
        info!(
            "Stage 5: Saving {} processed products to database",
            products.len()
        );
        ...
        info!(
            "Stage 5 completed: {} new, {} updated, {} errors",
            new_items, updated_items, errors
        );
