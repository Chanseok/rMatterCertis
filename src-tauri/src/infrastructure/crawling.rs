...
use crate::infrastructure::parsing::{
    ContextualParser, ParseContext, ParsingConfig, ProductDetailParser, ProductListParser,
};
...
impl WebCrawler {
    ...
    pub async fn crawl_product_list(&self, url: &str, page_id: u32) -> Result<Vec<Product>> {
        ...
        info!(
            "Successfully crawled {} products from page {}",
            products.len(),
            page_id
        );
        Ok(products)
    ...
    pub async fn crawl_product_detail(&self, url: &str) -> Result<ProductDetail> {
        ...
        info!(
            "Successfully crawled product detail: {}",
