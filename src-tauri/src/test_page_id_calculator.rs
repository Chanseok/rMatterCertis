...
mod tests {
    ...
    fn test_page_id_calculator_with_correct_values() {
        ...
        println!(
            "Page 1, Index 0: page_id={}, index_in_page={}",
            result1.page_id, result1.index_in_page
        );

        // 페이지 1, 인덱스 11 (12번째 제품)
        let result2 = calculator.calculate(1, 11);
        println!(
            "Page 1, Index 11: page_id={}, index_in_page={}",
            result2.page_id, result2.index_in_page
        );

        // 페이지 2, 인덱스 0 (두 번째 페이지 첫 번째 제품)
        let result3 = calculator.calculate(2, 0);
        println!(
            "Page 2, Index 0: page_id={}, index_in_page={}",
            result3.page_id, result3.index_in_page
        );

        // 페이지 100, 인덱스 0 (마지막 페이지 첫 번째 제품)
        let result4 = calculator.calculate(100, 0);
        println!(
            "Page 100, Index 0: page_id={}, index_in_page={}",
            result4.page_id, result4.index_in_page
        );

        // 페이지 100, 인덱스 7 (마지막 페이지 마지막 제품)
        let result5 = calculator.calculate(100, 7);
        println!(
            "Page 100, Index 7: page_id={}, index_in_page={}",
    ...
    fn test_page_id_calculator_old_vs_new() {
        ...
        println!(
            "Old method: page_id={}, index_in_page={}",
            old_result.page_id, old_result.index_in_page
        );
        ...
        println!(
            "New method: page_id={}, index_in_page={}",
            new_result.page_id, new_result.index_in_page
        );
