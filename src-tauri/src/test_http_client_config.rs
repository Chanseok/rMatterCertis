    ...
    if let Ok(config_manager) = config_manager {
        if let Ok(loaded_config) = config_manager.load_config().await {
            ...
            println!(
                ...
                loaded_config.user.crawling.workers.max_requests_per_second
            );
            println!(
                "  request_timeout_seconds: {}",
                loaded_config.user.crawling.workers.request_timeout_seconds
            );
            println!(
                "  max_retries: {}",
                loaded_config.user.crawling.workers.max_retries
            );
            println!(
                "  user_agent: {}",
                loaded_config.user.crawling.workers.user_agent
            );
            println!(
                "  follow_redirects: {}",
                loaded_config.user.crawling.workers.follow_redirects
            );
        }
    }
}
