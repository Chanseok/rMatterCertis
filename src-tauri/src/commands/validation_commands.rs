...
#[derive(Debug)]
struct DetectedAnomaly { code: String, detail: String }

fn detect_page_anomalies(physical_page: u32, product_urls: &[String], is_oldest_page: bool, items_on_last_page: usize) -> Vec<DetectedAnomaly> {
    let mut anomalies = Vec::new();
    // 1. duplicate_index (duplicate URL occurrences)
    use std::collections::HashMap;
    let mut counts = HashMap::new();
    for u in product_urls.iter() { *counts.entry(u).or_insert(0usize) += 1; }
    for (u,c) in counts.iter() { if *c > 1 { anomalies.push(DetectedAnomaly { code: "duplicate_index".into(), detail: format!("physical_page={} url={} count={}", physical_page, u, c) }); } }
    // 2. sparse_page (non-oldest page with < full items)
    if !is_oldest_page && product_urls.len() < SITE_PRODUCTS_PER_PAGE {
        anomalies.push(DetectedAnomaly { code: "sparse_page".into(), detail: format!("physical_page={} found={} expected={}", physical_page, product_urls.len(), SITE_PRODUCTS_PER_PAGE) });
    }
    // 3. oldest page size mismatch (oldest page size > expected or zero)
    if is_oldest_page {
        if product_urls.len() > items_on_last_page { anomalies.push(DetectedAnomaly { code: "oldest_page_overflow".into(), detail: format!("physical_page={} items={} expected_last_page_items={}", physical_page, product_urls.len(), items_on_last_page) }); }
        if product_urls.is_empty() { anomalies.push(DetectedAnomaly { code: "oldest_page_empty".into(), detail: format!("physical_page={} had zero products", physical_page) }); }
    }
    // 4. out_of_range (sanity: any absurdly long URL or missing required pattern)
    for u in product_urls.iter() { if !u.contains("csa_product") { anomalies.push(DetectedAnomaly { code: "unexpected_url_pattern".into(), detail: format!("physical_page={} url={} missing csa_product pattern", physical_page, u) }); }}
    anomalies
}
...
#[derive(Debug, serde::Serialize, serde::Deserialize, Clone)]
pub struct GapRange { pub start_offset: u64, pub end_offset: u64, pub size: u64 }
...
fn emit_actor_event(app: &AppHandle, event: AppEvent) {
    ...
    if let Ok(raw) = serde_json::to_value(&event) {
        ...
        if let Some(o) = enriched.as_object_mut() {
            o.insert("seq".into(), Value::from(VALIDATION_SEQ.fetch_add(1, Ordering::SeqCst)));
...
pub async fn start_validation(
    ...
    info!("Starting validation pass: session_id={} (pending scan_pages determination) oldest-forward", session_id);
    ...
    let newest_html = match http.fetch_response(&newest_url).await {
        Ok(resp) => match resp.text().await { Ok(t) => t, Err(e) => return Err(format!("Read newest page text error: {e}")) },
    ...
    let oldest_html = if oldest_physical_page == 1 { newest_html.clone() } else {
        match http.fetch_response(&oldest_url).await {
            Ok(resp) => match resp.text().await { Ok(t) => t, Err(e) => return Err(format!("Read oldest page text error: {e}")) },
            Err(e) => { return Err(format!("Failed to fetch oldest page {}: {e}", oldest_physical_page)); }
        }
    };
    let oldest_urls = extractor.extract_product_urls_from_content(&oldest_html).map_err(|e| e.to_string())?;
    ...
    let (scan_pages_used, physical_range_start_oldest, physical_range_end_newest) = if let Some((raw_start_opt, raw_end_opt)) = pending_custom_range {
        ...
        let mut pages_to_scan = if let Some(user) = user_scan_pages { user.max(1) } else {
            ...
            let total_products: i64 = sqlx::query("SELECT COUNT(*) as cnt FROM products")
                ...
                .try_get::<i64, _>("cnt").unwrap_or(0);
            let max_page_id: Option<i64> = sqlx::query("SELECT MAX(page_id) as max_pid FROM products WHERE page_id IS NOT NULL")
                .fetch_one(&pool).await.ok()
                .and_then(|row| row.try_get::<Option<i64>, _>("max_pid").ok()).flatten();
    ...
    for physical_page in (physical_range_end_newest..=physical_range_start_oldest).rev() { // oldest -> newer (descending numbers)
        ...
        for a in page_anomalies.into_iter() {
            ...
            emit_actor_event(&app, AppEvent::ValidationAnomaly { session_id: session_id.clone(), code: a.code, detail: a.detail, timestamp: Utc::now() });
        }
        ...
        for (i, url) in product_urls.iter().enumerate() {
            ...
            if min_offset.map(|m| expected_offset < m).unwrap_or(true) { min_offset = Some(expected_offset); }
            if max_offset.map(|m| expected_offset > m).unwrap_or(true) { max_offset = Some(expected_offset); }
            ...
            if !seen_urls.insert(url.clone()) {
                ...
                emit_actor_event(&app, AppEvent::ValidationAnomaly { session_id: session_id.clone(), code: "cross_page_duplicate".into(), detail: format!("url={} physical_page={} duplicate_across_pages", url, physical_page), timestamp: Utc::now() });
            ...
            match row {
                None => {
                    ...
                    page_divergences += 1;
                    mismatch_missing += 1;
                    emit_actor_event(&app, AppEvent::ValidationDivergenceFound { session_id: session_id.clone(), physical_page, kind: "missing".to_string(), detail: format!("Missing url {} (expected page_id={}, index_in_page={})", url, calc_res.page_id, calc_res.index_in_page), expected_offset, timestamp: Utc::now() });
                    if divergence_samples.len() < 200 { divergence_samples.push(DivergenceSample { url: url.clone(), physical_page, kind: "missing".into(), expected_page_id: calc_res.page_id, expected_index_in_page: calc_res.index_in_page, db_page_id: None, db_index_in_page: None, detail: "missing in DB".into() }); }
                    highest_divergence_physical_page = Some(highest_divergence_physical_page.map(|h| h.max(physical_page)).unwrap_or(physical_page));
                    lowest_divergence_physical_page = Some(lowest_divergence_physical_page.map(|l| l.min(physical_page)).unwrap_or(physical_page));
                }
                Some(r) => {
                    let db_pid: Option<i64> = r.get("page_id");
                    let db_idx: Option<i64> = r.get("index_in_page");
                    match (db_pid, db_idx) {
                        (Some(p), Some(idx)) if p as i32 == calc_res.page_id && idx as i32 == calc_res.index_in_page => {
                            products_checked += 1;
                        }
                        (p, idx) => {
                            divergences += 1;
                            page_divergences += 1;
                            mismatch_coord += 1;
                            if let (Some(_), Some(iv)) = (p, idx) { shift_values.push(iv as i32 - calc_res.index_in_page); }
                            emit_actor_event(&app, AppEvent::ValidationDivergenceFound { session_id: session_id.clone(), physical_page, kind: "coord_mismatch".to_string(), detail: format!("URL {} db=({:?},{:?}) expected=({}, {})", url, p, idx, calc_res.page_id, calc_res.index_in_page), expected_offset, timestamp: Utc::now() });
                            if divergence_samples.len() < 200 { divergence_samples.push(DivergenceSample { url: url.clone(), physical_page, kind: "coord_mismatch".into(), expected_page_id: calc_res.page_id, expected_index_in_page: calc_res.index_in_page, db_page_id: p.map(|v| v as i32), db_index_in_page: idx.map(|v| v as i32), detail: format!("db=({:?},{:?}) expected=({}, {})", p, idx, calc_res.page_id, calc_res.index_in_page) }); }
                            highest_divergence_physical_page = Some(highest_divergence_physical_page.map(|h| h.max(physical_page)).unwrap_or(physical_page));
                            lowest_divergence_physical_page = Some(lowest_divergence_physical_page.map(|l| l.min(physical_page)).unwrap_or(physical_page));
                        }
                    }
        ...
        emit_actor_event(&app, AppEvent::ValidationPageScanned { session_id: session_id.clone(), physical_page, products_found: product_urls.len() as u32, assigned_start_offset: min_offset.unwrap_or(0), assigned_end_offset: max_offset.unwrap_or(0), timestamp: Utc::now() });
        // Pattern detection: if all shift_values identical and non-empty
        let mismatch_shift_pattern = if !shift_values.is_empty() && shift_values.iter().all(|v| *v == shift_values[0]) {
            Some(shift_values[0])
        } else { None };
    per_page_stats.push(PerPageStat { physical_page, products_found: product_urls.len() as u32, divergences: page_divergences, anomalies: page_anomaly_count, mismatch_shift_pattern, mismatch_missing, mismatch_coord });
        ...
        if let (Some(start_o), Some(prev_end)) = (min_offset, last_end_offset) {
            if start_o > prev_end + 1 { // gap
                let gap = GapRange { start_offset: prev_end + 1, end_offset: start_o - 1, size: start_o - prev_end - 1 };
                emit_actor_event(&app, AppEvent::ValidationDivergenceFound { session_id: session_id.clone(), physical_page, kind: "gap".into(), detail: format!("gap offsets {}..{} size={} before physical_page {}", gap.start_offset, gap.end_offset, gap.size, physical_page), expected_offset: gap.start_offset, timestamp: Utc::now() });
                gap_ranges.push(gap);
            }
        }
        if let Some(end_o) = max_offset { last_end_offset = Some(end_o); }
        pages_scanned += 1;
    }
