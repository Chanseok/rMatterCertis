...
pub async fn start_legacy_service_based_crawling(
    ...
    let batch_configs = vec![crate::crawl_engine::actors::types::BatchConfig {
        batch_size: (start_page - end_page + 1).min(5),
...
async fn execute_real_stage_4_storage(
    ...
    for (index, product_json) in stage3_items.iter().enumerate() {
        ...
        info!("  💾 Processing item {}/{}: {}", index + 1, stage3_items.len(), url);
...
fn convert_json_to_product_detail(
    ...
    let url = json.get("source_url").and_then(|v| v.as_str()).unwrap_or("").to_string();
    let device_type = json.get("certification_id").and_then(|v| v.as_str()).map(str::to_string);
    let certification_date = json.get("certification_date").and_then(|v| v.as_str()).map(str::to_string);
    let software_version = json.get("software_version").and_then(|v| v.as_str()).map(str::to_string);
    let hardware_version = json.get("hardware_version").and_then(|v| v.as_str()).map(str::to_string);
    let description = json.get("description").and_then(|v| v.as_str()).map(str::to_string);
    let manufacturer = json.get("manufacturer").and_then(|v| v.as_str()).map(str::to_string);
    let model = json.get("model").and_then(|v| v.as_str()).map(str::to_string);
    let certification_id = json.get("certification_id").and_then(|v| v.as_str()).map(str::to_string);
...
pub enum FrontendEvent {
    SessionStarted { session_id: String, total_batches: u32, timestamp: chrono::DateTime<chrono::Utc> },
    BatchStarted { session_id: String, batch_id: String, batch_index: u32, timestamp: chrono::DateTime<chrono::Utc> },
    BatchCompleted { session_id: String, batch_id: String, batch_index: u32, success: bool, timestamp: chrono::DateTime<chrono::Utc> },
    SessionCompleted { session_id: String, duration_ms: u64, timestamp: chrono::DateTime<chrono::Utc> },
}
