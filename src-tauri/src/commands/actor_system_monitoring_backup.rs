...
use tauri::Emitter; // AppHandle.emit() 메소드 사용을 위해 추가
...
pub async fn start_crawling_session(
    ...
    match start_real_actor_session(app_handle, state).await {
        Ok(session_id) => {
...
/// 실제 Actor 시스템 세션 시작
async fn start_real_actor_session(
    app_handle: tauri::AppHandle,
    state: tauri::State<'_, crate::application::AppState>
) -> Result<String, String> {
    ...
    // 실제 크롤링 서비스 시작을 위한 백그라운드 태스크
    let session_id_clone = session_id.clone();
    let app_handle_clone = app_handle.clone();
    tokio::spawn(async move {
        info!("🚀 Starting real crawling execution for session: {}", session_id_clone);
        ...
        let initial_event = crate::new_architecture::events::task_lifecycle::ConcurrencyEvent::SessionEvent {
            ...
            metadata: std::collections::HashMap::from([
                ("message".to_string(), "Actor system crawling session started".to_string()),
            ]),
            timestamp: chrono::Utc::now(),
        };
        
        if let Err(e) = app_handle_clone.emit("concurrency-event", &initial_event) {
            error!("❌ Failed to emit initial session event: {}", e);
        } else {
            info!("🔔 Emitted SessionEvent::Started for session: {}", session_id_clone);
        }
        
        // 배치 계획 수립 이벤트 (1초 후)
        tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        
        let batch_planning_event = crate::new_architecture::events::task_lifecycle::ConcurrencyEvent::SessionEvent {
            session_id: session_id_clone.clone(),
            event_type: crate::new_architecture::events::task_lifecycle::SessionEventType::BatchPlanning,
            metadata: std::collections::HashMap::from([
                ("message".to_string(), "Planning batches".to_string()),
            ]),
            timestamp: chrono::Utc::now(),
        };
        
        if let Err(e) = app_handle_clone.emit("concurrency-event", &batch_planning_event) {
            error!("❌ Failed to emit batch planning event: {}", e);
        } else {
            info!("🔔 Emitted SessionEvent::BatchPlanning");
        }
        
        // 사이트 상태 확인 이벤트 (2초 후)
        tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        
        let site_check_event = crate::new_architecture::events::task_lifecycle::ConcurrencyEvent::SessionEvent {
            session_id: session_id_clone.clone(),
            event_type: crate::new_architecture::events::task_lifecycle::SessionEventType::SiteStatusCheck,
            metadata: std::collections::HashMap::from([
                ("message".to_string(), "Checking site status".to_string()),
            ]),
            timestamp: chrono::Utc::now(),
        };
        
        if let Err(e) = app_handle_clone.emit("concurrency-event", &site_check_event) {
            error!("❌ Failed to emit site check event: {}", e);
        } else {
            info!("🔔 Emitted SessionEvent::SiteStatusCheck");
        }
        
        // 실제 Smart Crawling 호출 (3초 후)
        tokio::time::sleep(std::time::Duration::from_secs(1)).await;
        ...
        match crate::commands::simple_crawling::start_smart_crawling(
            ...
            Ok(_) => {
                ...
                let completion_event = crate::new_architecture::events::task_lifecycle::ConcurrencyEvent::SessionEvent {
                    ...
                    metadata: std::collections::HashMap::from([
                        ("message".to_string(), "Crawling completed successfully".to_string()),
                    ]),
                    timestamp: chrono::Utc::now(),
                };
            ...
            Err(e) => {
                ...
                let failure_event = crate::new_architecture::events::task_lifecycle::ConcurrencyEvent::SessionEvent {
                    ...
                    metadata: std::collections::HashMap::from([
                        ("message".to_string(), format!("Crawling failed: {}", e)),
                        ("error".to_string(), e),
                    ]),
                    timestamp: chrono::Utc::now(),
                };
                ...
                if let Err(e) = app_handle_clone.emit("concurrency-event", &failure_event) {
                    error!("❌ Failed to emit failure event: {}", e);
                } else {
