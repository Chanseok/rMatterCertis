...
pub async fn start_partial_sync(
    ...
    if ranges.is_empty() {
        return Err("No valid ranges provided".into());
    }
        ...
        for r in ranges.iter_mut() {
            ...
            if let Some(limit) = limit_opt {
                ...
                if span > limit {
                    ...
                    info!(
                        "Clamping sync span from {} to {} by config validation_page_limit={}, range {}->{}, new {}->{}",
    ...
    info!(
        "Sync started: session_id={} ranges={:?} dry_run={}",
        session_id,
        ranges,
    ...
    for (start_oldest, end_newest) in ranges.into_iter() {
        ...
        for physical_page in (end_newest..=start_oldest).rev() {
            // oldest->newer
            emit_actor_event(
                &app,
            ...
            let page_html = if physical_page == oldest_page {
                oldest_html.clone()
            ...
            let product_urls = match extractor.extract_product_urls_from_content(&page_html) {
                Ok(v) => v,
                Err(e) => {
                    failed += 1;
                    emit_actor_event(
            ...
            for (i, url) in product_urls.iter().enumerate() {
                ...
                match row {
                    ...
                    Some(r) => {
                        ...
                        if needs_update {
                            let res = sqlx::query(
                                "UPDATE products SET page_id = ?, index_in_page = ? WHERE url = ?",
                            )
                            .bind(calc.page_id)
                            .bind(calc.index_in_page)
                            .bind(url)
