...
) -> Result<(ExecutionPlan, AppConfig, DomainSiteStatus), Box<dyn std::error::Error + Send + Sync>>
{
	...
	match product_repo.get_product_count().await {
		Ok(count) => { info!("✅ Database connection successful: {} products found", count); }
		Err(e) => { error!("❌ Database connection failed in create_execution_plan: {}", e); return Err(format!("Database connection test failed: {}", e).into()); }
	}
		...
		shared_cache.as_ref()
	{
		...
		} else { info!("🔄 No valid cached SiteStatus (or expired) – performing fresh check"); None }
	} else { info!("📭 SharedStateCache not available in Tauri state – proceeding without cache"); None };

	let existing_product_count = match product_repo.get_product_count().await { Ok(c) => c, Err(e) => { warn!(
	...
	if existing_product_count > 0 { chosen_strategy = crate::crawl_engine::actors::types::CrawlingStrategy::ContinueFromDb; info!("🧭 Choosing ContinueFromDb strategy (existing products={})", existing_product_count); } else { info!("🧭 Choosing NewestFirst strategy (empty DB)"); }
	...
	let head: Vec<String> = page_slots
		.iter()
		.take(5)
		.map(|s| { format!("p{}=>gid{}_i{}", s.physical_page, s.page_id, s.index_in_page) })
		.collect();
...
async fn build_execution_plan_from_explicit_pages(
	...
	pages.dedup();
	if pages.is_empty() { return Err("No valid pages after normalization".into()); }
	...
	while i < pages.len() {
		...
		let mut j = i;
		while j + 1 < pages.len() && pages[j] == pages[j + 1] + 1 { j += 1; }
		let end_oldest = pages[j];
		let count = start_newest.saturating_sub(end_oldest) + 1;
		ranges.push(PageRange { start_page: start_newest, end_page: end_oldest, estimated_products: count * 12, reverse_order: true });
		i = j + 1;
	}
	...
	for r in &ranges {
		...
		for physical_page in iter {
			...
			for idx in 0..assumed_capacity {
				let pos = calc.calculate(physical_page, idx, site_status.total_pages);
				page_slots.push(crate::crawl_engine::actors::types::PageSlot { physical_page, page_id: i64::from(pos.page_id), index_in_page: i16::try_from(pos.index_in_page).unwrap_or(i16::MAX) });
			}
		}
	}
	...
	if let Some(ref mut kpi) = execution_plan.kpi_meta { kpi.total_ranges = execution_plan.crawling_ranges.len(); kpi.batches = execution_plan.crawling_ranges.len(); }
	Ok((execution_plan, app_config, site_status))
}
...
pub async fn start_manual_crawl_pages_actor(
	...
	if pages.is_empty() { return Err("No pages provided".into()); }
