...
pub async fn analyze_system_status(
    ...
    info!("🧠 Backend now remembers: {} total pages, DB cursor at {}:{}", 
          site_analysis.total_pages,
          db_analysis.max_page_id.unwrap_or(-1),
          db_analysis.max_index_in_page.unwrap_or(-1));
...
async fn perform_site_analysis() -> Result<SiteAnalysisResult, String> {
    ...
    info!("📊 Site analysis completed: {} pages, {} products on last page, {} estimated total", 
...
async fn perform_database_analysis() -> Result<DbAnalysisResult, String> {
    ...
    info!("📊 Database analysis completed: {} products, cursor at {}:{}", 
...
pub async fn get_analysis_cache_status(
    ...
    let site_status = shared_state.get_valid_site_analysis_async(Some(ttl::SITE_ANALYSIS_TTL_MINUTES)).await;
    let db_status = shared_state.get_valid_db_analysis_async(Some(ttl::DB_ANALYSIS_TTL_MINUTES)).await;
    let range_status = shared_state.get_valid_calculated_range_async(ttl::CALCULATED_RANGE_TTL_MINUTES).await;
