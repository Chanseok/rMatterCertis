...
#[derive(Debug, Clone, Deserialize)]
struct CsaVendorsPage {
    #[serde(rename = "vendorInfo")]
    vendor_info: Vec<CsaVendorItem>,
    pagination: CsaPagination,
}

async fn fetch_csa_page(
    http: &crate::infrastructure::simple_http_client::HttpClient,
    key: Option<&str>,
) -> anyhow::Result<CsaVendorsPage> {
    let base = "https://on.dcl.csa-iot.org/dcl/vendorinfo/vendors";
    let url = if let Some(k) = key {
        format!("{}?pagination.key={}", base, k)
    } else {
        base.to_string()
    };
...
pub async fn sync_vendors_internal(
    ...
    let mut next_key = if api_total > local_count {
        first.pagination.next_key.clone()
    } else {
        None
    };
    ...
    for v in &first.vendor_info {
            match repo
                .upsert_vendor_by_number(
                    v.vendor_id,
                    &v.vendor_name,
                    v.company_legal_name.as_deref(),
                )
                .await
            {
                Ok(UpsertOutcome {
                ...
                Err(e) => {
                    warn!(
                        "Vendor upsert failed for {} ({}): {}",
                        v.vendor_name, v.vendor_id, e
                    );
                }
            }
        }
