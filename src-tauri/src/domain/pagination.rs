...
impl Default for PaginationCalculator {
    fn default() -> Self { Self { products_per_page: PRODUCTS_PER_PAGE } }
}

impl PaginationCalculator {
    pub fn new(products_per_page: usize) -> Self { Self { products_per_page } }
    ...
    pub fn calculate(&self, physical_page: u32, index_in_physical: u32, total_pages: u32) -> PagePosition {
        // 방어 로직
        if total_pages == 0 { return PagePosition { page_id: 0, index_in_page: 0 }; }
        ...
        let from_oldest_product_number = (distance_from_oldest_page as usize) * self.products_per_page + index_in_physical as usize;
        ...
        PagePosition { page_id, index_in_page }
    }

    /// reverse: page_id / index_in_page 로부터 물리 페이지와 해당 페이지 내 index 추정.
    /// 일부 정보 손실(마지막/이전 경계)에 의해 모호성이 있을 수 있어 Option 반환.
    pub fn reverse(&self, page_id: i32, index_in_page: i32, total_pages: u32) -> Option<(u32, u32)> {
        if page_id < 0 || index_in_page < 0 || index_in_page as usize >= self.products_per_page { return None; }
        let from_oldest_product_number = (page_id as usize) * self.products_per_page + index_in_page as usize;
        let distance_from_oldest_page = from_oldest_product_number / self.products_per_page; // 페이지 경계
        if distance_from_oldest_page as u32 >= total_pages { return None; }
