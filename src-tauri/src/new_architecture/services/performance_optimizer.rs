...
impl CrawlingPerformanceOptimizer {
    ...
    fn determine_optimization_status(
        ...
        } else if avg_response_time_ms < target_response_time * 0.5 && current_concurrency < max_concurrency {
            // 충분히 빠름, 동시성 증가 가능
    ...
    pub async fn get_optimization_recommendation(&self) -> Option<OptimizationRecommendation> {
        ...
        let reason = match current_metrics.optimization_status {
            ...
            OptimizationStatus::ShouldIncrease => "동시성을 증가시켜 처리량을 향상시킬 수 있습니다".to_string(),
            OptimizationStatus::ShouldDecrease => "동시성을 감소시켜 안정성을 향상시켜야 합니다".to_string(),
            OptimizationStatus::Unstable => "시스템이 불안정합니다. 설정을 보수적으로 조정하세요".to_string(),
        };
