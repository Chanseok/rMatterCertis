...
impl StageActor {
    ...
    pub async fn new_with_real_crawling_service(
        ...
        let mut stage_actor = Self::new_with_oneshot(batch_id, config, total_pages, products_on_last_page);
    ...
    pub async fn run_with_real_crawling(
        ...
        while let Some(command) = control_rx.recv().await {
            match command {
                ...
                crate::new_architecture::channels::types::ActorCommand::CancelSession { reason, .. } => {
...
impl crate::new_architecture::actors::BatchActor {
    ...
    pub async fn execute_stage_with_real_crawling(
        ...
        // 6. 스테이지 명령 전송
        let stage_timeout = std::time::Duration::from_secs(
            config_arc.actor.stage_timeout_secs
        );
        let command = crate::new_architecture::channels::types::ActorCommand::ExecuteStage {
            stage_type: stage_type.clone(),
            items,
            concurrency_limit: config_arc.performance.concurrency.stage_concurrency_limits
