...
impl CrawlingEngineFactory {
    ...
    pub async fn create_engine(
        ...
        // 4. 오케스트레이터 설정 (개발 용이성을 위해 기본값 사용)
        let orchestrator_config = OrchestratorConfig {
            max_global_concurrency: config.max_concurrent_requests,
            scheduler_interval: Duration::from_millis(100), // 기본값
            shutdown_timeout: Duration::from_secs(30), // 기본값
            stats_interval: Duration::from_secs(10), // 기본값
            auto_retry_enabled: true, // 기본값
            max_retries: config.max_retries,
            retry_delay: Duration::from_millis(1000), // 기본값
            backpressure_enabled: false, // 기본값 (개발 단계에서 비활성화)
            backpressure_threshold: 1000, // 기본값
        };
    ...
    pub async fn start_crawling_session(
        ...
        for page in start_page..=end_page {
            let task = CrawlingTask::FetchListPage {
                task_id: TaskId::new(),
                page_number: page,
                url: format!("{}/page/{}", self.config.base_url, page),
            };
