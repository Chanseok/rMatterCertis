...
pub use queues::{QueueError, QueueManager, TaskQueue};
pub use state::{CrawlingConfig, CrawlingStats, SharedState};
pub use tasks::{CrawlingTask, TaskId, TaskOutput, TaskProductData, TaskResult};
pub use workers::{WorkerError, WorkerPool, WorkerPoolBuilder, WorkerPoolStats};
...
impl CrawlingEngineFactory {
    ...
    pub async fn create_engine(
        ...
        Err(CrawlingEngineError::ConfigurationError(
            "Factory pattern is deprecated. Use with_config or with_config_and_db instead."
                .to_string(),
        ))
    }
}
...
impl CrawlingEngine {
    ...
    pub async fn start_crawling_session(
        ...
        tracing::info!(
            "🚀 Starting crawling session: pages {} to {}",
            start_page,
            end_page
        );
    ...
    pub async fn with_config(config: CrawlingConfig) -> Result<Self, CrawlingEngineError> {
        ...
        .await
        .map_err(|e| {
            CrawlingEngineError::DatabaseConnectionError(format!("Failed to create tables: {}", e))
        })?;
