...
use crate::domain::session_manager::{
    CrawlingSessionState, CrawlingStage, SessionManager, SessionStatus,
};
...
impl CrawlingUseCases {
    ...
    pub async fn start_crawling_session(&self, config: CrawlingConfig) -> Result<String> {
        ...
        let _session_id = self
            .session_manager
            .start_session(config_json, config.max_pages, CrawlingStage::ProductList)
            .await;
        ...
        info!(
            "Crawling session {} started - ready for processing",
            session_id
        );
        Ok(session_id)
    }
    ...
    pub async fn stop_crawling_session(&self, session_id: &str) -> Result<()> {
        ...
        self.session_manager
            .set_status(session_id, SessionStatus::Stopped)
            .await
...
mod tests {
    ...
    async fn test_use_cases_creation() {
        ...
        let use_cases =
            CrawlingUseCases::new(http_client, data_extractor, product_repo, session_manager);
