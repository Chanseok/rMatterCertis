...
use crate::infrastructure::parsing::{
    ParsingConfig, ProductListParser, ProductDetailParser, 
    ParseContext, ParsingResult, ContextualParser
};
...
impl ParsingService {
    ...
    pub fn parse_product_list(&self, html: &str, page_id: u32) -> ParsingResult<Vec<Product>> {
        ...
        info!("Successfully parsed {} products from page {}", products.len(), page_id);
        Ok(products)
    }
    ...
    pub fn parse_product_detail(&self, html: &str, url: &str) -> ParsingResult<ProductDetail> {
        ...
        info!("Successfully parsed product detail: {}", 
    ...
    pub fn validate_product(&self, product: &Product) -> ParsingResult<()> {
        if product.url.is_empty() {
            return Err(crate::infrastructure::parsing_error::ParsingError::ProductValidationFailed {
...
impl CrawlerService {
    ...
    pub fn new(config: ParsingConfig) -> Result<Self> {
        let crawler = WebCrawler::new(config.clone())
            .context("Failed to create web crawler")?;
        
        let parsing_service = Arc::new(
            ParsingService::new(config)
    ...
    pub async fn crawl_and_parse_product_list(&self, url: &str, page_id: u32) -> Result<Vec<Product>> {
